...
- Decision-ledger edit policy is a pure module (`packages/server/src/decisions.ts`): line-targeted splice, exactly-one-match, refuse-duplicates, writer-no-looser-than-reader grammar. Offline check: `npm run qa:decisions`.
