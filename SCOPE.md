...
8. **Reviews.** Review is an intake ceremony, not a storage noun (see DECISIONS.md). The user dumps batch feedback via the composer's Review mode (or the agent detects an unprompted dump and offers the ceremony): the raw text is saved verbatim to `.clyde/reviews/<batch>.md` as provenance → the agent distills it to numbered items and echoes them → clarifies ambiguities in one AskUserQuestion → confirms scope in one multiSelect question card → every item becomes a Task carrying `source`/`batch` provenance — accepted ones as pending, rejected ones as `declined` with reasons, nothing silently dropped. The Reviews panel is a burn-down lens over task batches (settled = completed + declined); legacy markdown checklists keep rendering. (Later: items thread-anchorable into the conversation.)
...
The spine is **one significance judgment**: the concrete trigger list in `.clyde/ENGINEERING.md` gates the entire apparatus — the list lives there alone, versioned and amendable, never duplicated. Ordinary tasks pay **zero** ceremony. In build order:
